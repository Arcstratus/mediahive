...
const { data: folders } = await bookmarksApi.getFolders('bookmark-folders')
...
<template>
  <div class="flex flex-col gap-6">
    <PageBreadcrumb :items="[{ label: 'Bookmarks', to: '/bookmarks' }, { label: 'Folders' }]" />
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          <UCard
            ...
            @click="navigateTo(`/bookmarks/folders/${encodeURIComponent(item.folder)}`)"
          >
            <div class="flex items-center gap-3">
              <UIcon name="i-lucide-folder" class="size-8 text-primary-500 shrink-0" />
              <div class="min-w-0">
                <p class="font-medium truncate">{{ item.folder }}</p>
                <p class="text-sm text-gray-500">{{ item.count }} bookmark{{ item.count === 1 ? '' : 's' }}</p>
              </div>
            </div>
          </UCard>
        </div>
