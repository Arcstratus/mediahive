...
<template>
  <div class="flex flex-col gap-6">
    <PageBreadcrumb :items="[{ label: 'Foundation', to: '/foundation' }, { label: '使用者管理' }]" />

    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">使用者管理</h1>
      <UButton label="新增使用者" icon="i-lucide-plus" to="/foundation/user/new" />
    </div>
      ...
      <template #actions-cell="{ row }">
        <div class="flex gap-1 justify-end">
          <UButton
            ...
            color="neutral"
            size="xs"
            :to="`/foundation/user/${row.original.uuid}`"
          />
        </div>
      </template>
    </UTable>
