...
async function onSubmit() {
  // TODO: 後端支援 - 呼叫 API 建立角色
  loading.value = true
  setTimeout(() => {
    loading.value = false
    toast.add({ title: '角色已建立（Demo）', color: 'success' })
    navigateTo('/foundation/role')
  }, 800)
}
</script>
      ...
      <UCard>
        <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
          ...
          <div class="flex justify-end gap-2 pt-4">
            <UButton label="取消" variant="outline" color="neutral" to="/foundation/role" />
