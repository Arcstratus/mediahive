...
<template>
  <div class="flex flex-col gap-6">
    <PageBreadcrumb :items="[{ label: 'Foundation', to: '/foundation' }, { label: '角色權限' }]" />

    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">角色權限</h1>
      <UButton label="新增角色" icon="i-lucide-plus" to="/foundation/role/new" />
    </div>
    ...
    <UTable :data="roles" :columns="columns">
      ...
      <template #actions-cell="{ row }">
        <div class="flex gap-1 justify-end">
          <UButton icon="i-lucide-pencil" variant="ghost" color="neutral" size="xs" :to="`/foundation/role/${row.original.id}`" />
        </div>
      </template>
    </UTable>
  </div>
