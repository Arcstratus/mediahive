...
const { data: folders } = await resourcesApi.getFolders('viewer-folders')
...
async function fetchIds() {
  ...
  const { data } = await resourcesApi.getIds({ folder: folder.value })
...
function preloadMedia(res: Resource) {
  ...
  const f = res.folder ? `${res.folder}/` : ''
  const fullUrl = `${apiBase}/media/${f}${res.filename}`
...
async function deleteResource() {
  ...
  if (ids.value.length === 0) {
    router.replace('/resources/folders')
    return
  }
  ...
  <div class="flex flex-col gap-6">
    <PageBreadcrumb
      :items="[
        ...
        { label: 'Folders', to: '/resources/folders' },
        { label: folder },
      ]"
    />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
      ...
      <div>
        <div v-if="!folder" class="text-center text-gray-500 py-12">
          No folder specified. <NuxtLink to="/resources/folders" class="text-primary-500 underline">Browse folders</NuxtLink>
        </div>
        ...
        <div v-else-if="resource" class="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
