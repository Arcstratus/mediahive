...
<template>
  <div class="flex flex-col gap-6">
    <PageBreadcrumb :items="[{ label: '組織架構' }]" />

    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">組織架構</h1>
      <UButton label="新增部門" icon="i-lucide-plus" to="/department/new" />
    </div>
    ...
    <UTable v-if="tab === 'list'" :data="departments" :columns="columns">
      ...
      <template #actions-cell="{ row }">
        <div class="flex gap-1 justify-end">
          <UButton icon="i-lucide-pencil" variant="ghost" color="neutral" size="xs" :to="`/department/${row.original.id}`" />
        </div>
      </template>
    </UTable>
